...
public function updateInvoiceStatusAsBackDelivered($id)
{
    ...
    public function AccountsUpdateLoad(Request $request, $id)
    {
       // dd($request->all());
        ...
        $load->invoice_number = $load->invoice_number ?? ''  ;
        $load->invoice_date = $load->invoice_date ?? '';        
    ...
    //         // Generate a boundary for separating parts
    //         $boundary = md5(time());
    
    //         // Initialize headers for HTML email
    //         $headers = "From: ar@cargoconvoy.co\r\n";
    //         $headers .= "Reply-To: ar@cargoconvoy.co\r\n";
...
public function invoiceSendEmail(Request $request)
{
    ...
    $validator = Validator::make($request->all(), [
        ...
        'subject' => 'required|string',
        'message' => 'required|string',
        'selected_attachments' => 'nullable|array',  // Validate if it's an array
        'selected_attachments.*' => 'file|max:10000', // Allow any file type (remove mime types restriction)
    ]);
    ...
    try {
        $toEmail = $request->input('toEmail');  // Recipient's email
        $subject = $request->input('subject');  // Email subject
        $message = $request->input('message');  // Email body message

        // Decode the JSON encoded file paths
        $attachments = json_decode($request->input('selected_attachments', '[]'), true);  
        ...
        // If attachments exist, process and attach them
        if (!empty($attachments)) {
            $attachmentFiles = [];
            foreach ($attachments as $filePath) {
                $attachmentFiles[] = storage_path('app/public/uploads/email_attachments/' . basename($filePath)); // Process the file path
            }

            // Send email with attachments
            Mail::send('emails.invoiceEmail', $emailData, function ($mail) use ($emailData, $attachmentFiles) {
                $mail->from('info@cargoconvoy.co', 'Cargo Convoy');
                $mail->to($emailData['toEmail']);
                $mail->subject($emailData['subject']);
                $mail->setBody($emailData['message'], 'text/html'); // Email content
            ...
            // Send email without attachments if no files are selected
            Mail::send('emails.invoiceEmail', $emailData, function ($mail) use ($emailData) {
                $mail->from('info@cargoconvoy.co', 'Cargo Convoy');
                $mail->to($emailData['toEmail']);
                $mail->subject($emailData['subject']);
                $mail->setBody($emailData['message'], 'text/html'); // Email content
        ...
        $emailRecord = new InvoicesEmail();
        $emailRecord->from_email = 'info@cargoconvoy.co';
        $emailRecord->to_email = $toEmail;
        $emailRecord->subject = $subject;
        $emailRecord->message = $message;
        $emailRecord->attachments = json_encode($attachmentFiles);
        $emailRecord->save();
